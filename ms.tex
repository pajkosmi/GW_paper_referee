...
\textcolor{red}{[1] Compared to previous works, the strength of this project is its ability analyze GWs from multiple progenitors hundreds of milliseconds post bounce while accurately accounting for rotation and neutrinos.  }
...
Our results in the previous section support the efficacy of our effective GR potential for accurately modeling the GW signals from CCSNe \textcolor{red}{[8]during the bounce and early post bounce phase.  While the effective GR potential has been shown to underestimate peak frequency from GWs compared to GR, it accurately captures the GW amplitude and PNS compactness during the accretion phase \citep{muller:2013}.  Thus, we now turn to exploring the rotational effects on the GW signal during the accretion phase, up to 300 ms after bounce.}
...
Furthermore, we recast our analysis by focusing on regions within the CCSN that emit GWs.  \textcolor{red}{[10]Figure \ref{fig:region} displays the GW strain emitted from the 20 $M_\odot$ progenitor from the inner 50 km (upper panel) and from the region between 50 km and 150 km (lower panel) of the supernova.  The upper panel shows an initial, low frequency signal, independent of rotation within the first 60 ms post bounce.  This signal is typically characterized by prompt convection \citep{mueller:1997}.}  The lower panel of Figure \ref{fig:region} displays the inhibited convective signal \textcolor{red}{($t_{pb} > 100$ ms)} with increasing rotation, as the GW signal in the gain region becomes increasingly muted.  \textcolor{red}{\st{The typical convective signals in the early post bounce regime are then quickly washed out by the post bounce ring down of the PNS, as rotation increases.}}
...
the University of Chicago.  
    ...
    \software{\\\href{http://flash.uchicago.edu/site/}{FLASH} \citep{fryxell:2000,fryxell:2010},\\ \href{https://matplotlib.org/}{Matplotlib} \citep{hunter:2007},\\ \href{http://www.numpy.org/}{NumPy} \citep{vanderwalt:2011},\\ \href{https://www.scipy.org/}{SciPy} \citep{jones:2001}}
